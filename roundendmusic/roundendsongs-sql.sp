...
public Plugin:myinfo = {
    ...
    author = "nosoop",
    description = "Extension of the Round End Music plugin to support SQL databases.",
...
// SQL random function -- returns a even floating-point value between 0.0 and 1.0.
new String:g_sRandomFunction[] = "rand()";

public OnPluginStart() {
    g_hCDatabaseName = CreateConVar("sm_rem_sql_db", "roundendsongs", "Database entry in databases.cfg to load songs from.", FCVAR_PLUGIN|FCVAR_SPONLY);
    PrepareStringConVar(g_hCDatabaseName, g_sDatabaseName, sizeof(g_sDatabaseName));
    
    g_hCTableName = CreateConVar("sm_rem_sql_tbl", "songdata", "Table to read songs from.", FCVAR_PLUGIN|FCVAR_SPONLY);
    PrepareStringConVar(g_hCTableName, g_sTableName, sizeof(g_sTableName));
    
    g_hCSongDir = CreateConVar("sm_rem_sql_dir", "roundendsongs/", "Directory that the songs are located in, if not already defined as the full path in the database.", FCVAR_PLUGIN|FCVAR_SPONLY);
...
/**
 * Gets a handle on the SQL database.
 */
Handle:GetSongDatabaseHandle() {
    // TODO Verify that this is SQL and not SQLite.
    ...
    if (SQL_CheckConfig(g_sDatabaseName)) {
        ...
        if ( (hDatabase = SQL_Connect(g_sDatabaseName, true, sErrorBuffer, sizeof(sErrorBuffer))) == INVALID_HANDLE ) {
            SetFailState("[rem-sql] Could not connect to Round End Songs database: %s", sErrorBuffer);
        } else {
            return hDatabase;
        }
    } else {
        SetFailState("[rem-sql] Could not find configuration %s for the Round End Songs database.", g_sDatabaseName);
    }
