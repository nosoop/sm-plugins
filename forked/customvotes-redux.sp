...
new VoteType:g_iVoteType[MAX_VOTE_TYPES];
...
{
	CreateConVar("sm_customvotes_version", PLUGIN_VERSION, PLUGIN_NAME, FCVAR_PLUGIN | FCVAR_SPONLY | FCVAR_DONTRECORD | FCVAR_NOTIFY);
...
public MenuHandler_PlayersVote(Handle:hMenu, MenuAction:iAction, iVoter, iParam2)
{
	...
	if(iAction == MenuAction_Select)
	{
		...
		if(g_strVoteCallNotify[iVote][0])
		{
			...
			FormatVoteString(iVote, false, iTarget, strNotification, sizeof(strNotification));
			FormatVoterString(iVote, false, iVoter, strNotification, sizeof(strNotification));
			FormatTargetString(iVote, false, iTarget, strNotification, sizeof(strNotification));
		...
		if(!IsFakeClient(iTarget) && IsClientAuthorized(iTarget))
		{
			...
			GetClientAuthString(iVoter, strAuth, sizeof(strAuth));
...
public Vote_Players(iVote, iVoter, iTarget)
{
	...
	GetClientAuthString(iTarget, g_strVoteTargetAuth, sizeof(g_strVoteTargetAuth));
	...
	if(iAction == MenuAction_Select)
	{
		...
		if(StrEqual(strInfo, "Yes"))
		{
			...
			if(g_strVoteCallNotify[g_iCurrentVoteIndex][0])
			{
				...
				FormatVoteString(g_iCurrentVoteIndex, false, g_iCurrentVoteTarget, strNotification, sizeof(strNotification));
				FormatVoterString(g_iCurrentVoteIndex, false, iVoter, strNotification, sizeof(strNotification));
				FormatTargetString(g_iCurrentVoteIndex, false, g_iCurrentVoteTarget, strNotification, sizeof(strNotification));
...
public MenuHandler_MapVote(Handle:hMenu, MenuAction:iAction, iVoter, iParam2)
{
	...
	if(iAction == MenuAction_Select)
	{
		...
		if(g_strVoteCallNotify[iVote][0])
		{
			...
			FormatVoteString(iVote, false, iMap, strNotification, sizeof(strNotification));
			FormatVoterString(iVote, false, iVoter, strNotification, sizeof(strNotification));
			FormatMapString(iVote, false, iMap, strNotification, sizeof(strNotification));
	...
	if(iAction == MenuAction_Select)
	{
		...
		if(StrEqual(strInfo, "Yes"))
		{
			...
			if(g_strVoteCallNotify[g_iCurrentVoteIndex][0])
			{
				...
				FormatVoteString(g_iCurrentVoteIndex, false, g_iCurrentVoteMap, strNotification, sizeof(strNotification));
				FormatVoterString(g_iCurrentVoteIndex, false, iVoter, strNotification, sizeof(strNotification));
				FormatMapString(g_iCurrentVoteIndex, false, g_iCurrentVoteMap, strNotification, sizeof(strNotification));
...
public MenuHandler_ListVote(Handle:hMenu, MenuAction:iAction, iVoter, iParam2)
{
	...
	if(iAction == MenuAction_Select)
	{
		...
		if(g_strVoteCallNotify[iVote][0])
		{
			...
			FormatVoteString(iVote, false, iOption, strNotification, sizeof(strNotification));
			FormatVoterString(iVote, false, iVoter, strNotification, sizeof(strNotification));
			FormatOptionString(iVote, false, iOption, strNotification, sizeof(strNotification));
	...
	if(iAction == MenuAction_Select)
	{
		...
		if(StrEqual(strInfo, "Yes"))
		{
			...
			if(g_strVoteCallNotify[g_iCurrentVoteIndex][0])
			{
				...
				FormatVoteString(g_iCurrentVoteIndex, false, g_iCurrentVoteOption, strNotification, sizeof(strNotification));
				FormatVoterString(g_iCurrentVoteIndex, false, iVoter, strNotification, sizeof(strNotification));
				FormatOptionString(g_iCurrentVoteIndex, false, g_iCurrentVoteOption, strNotification, sizeof(strNotification));
...
public CastSimpleVote(iVote, iVoter)
{
	...
	if(g_strVoteCallNotify[iVote][0])
	{
		...
		FormatVoteString(iVote, false, _, strNotification, sizeof(strNotification));
		FormatVoterString(iVote, false, iVoter, strNotification, sizeof(strNotification));
	...
	if(iAction == MenuAction_Select)
	{
		...
		if(StrEqual(strInfo, "Yes"))
		{
			...
			if(g_strVoteCallNotify[g_iCurrentVoteIndex][0])
			{
				...
				FormatVoteString(g_iCurrentVoteIndex, false, _, strNotification, sizeof(strNotification));
				FormatVoterString(g_iCurrentVoteIndex, false, iVoter, strNotification, sizeof(strNotification));
...
public bool:CheckVotesForTarget(iVote, iTarget)
{
	...
	if(iVotes >= iRequired)
	{
		...
		if(g_strVoteCommand[iVote][0])
		{
			...
			FormatTargetString(iVote, true, iTarget, strCommand, sizeof(strCommand));
...
public bool:CheckVotesForMap(iVote, iMap)
{
	...
	if(iVotes >= iRequired)
	{
		...
		if(g_strVoteCommand[iVote][0])
		{
			...
			FormatMapString(iVote, false, iMap, strCommand, sizeof(strCommand));
			ServerCommand(strCommand);
		}
...
public bool:CheckVotesForOption(iVote, iOption)
{
	...
	if(iVotes >= iRequired)
	{
		...
		if(g_strVoteCommand[iVote][0])
		{
			...
			FormatOptionString(iVote, false, iOption, strCommand, sizeof(strCommand));
			ServerCommand(strCommand);
		}
...
public bool:CheckVotesForSimple(iVote)
{
	...
	if(iVotes >= iRequired)
	{
		...
		if(g_strVoteCommand[iVote][0])
		{
			...
			FormatVoteString(iVote, false, _, strCommand, sizeof(strCommand));
			ServerCommand(strCommand);
		}
...
stock FormatVoterString(iVote, bool:bQuotes = false, iVoter, String:strBuffer[], iBufferSize)
{
	decl String:strVoter[MAX_NAME_LENGTH];
	IntToString(iVoter, strVoter, sizeof(strVoter));

	if(bQuotes) QuoteString(strVoter, sizeof(strVoter));
	...
	if(bQuotes) QuoteString(strVoterId, sizeof(strVoterId));
	ReplaceString(strBuffer, iBufferSize, "{VOTER_ID}", strVoterId, false);

	decl String:strVoterSteamId[MAX_NAME_LENGTH];
	GetClientAuthString(iVoter, strVoterSteamId, sizeof(strVoterSteamId));

	if(bQuotes) QuoteString(strVoterSteamId, sizeof(strVoterSteamId));
	...
	if(bQuotes) QuoteString(strVoterName, sizeof(strVoterName));
	ReplaceString(strBuffer, iBufferSize, "{VOTER_NAME}", strVoterName, false);
}

stock FormatVoteString(iVote, bool:bQuotes = false, iChoice = -1, String:strBuffer[], iBufferSize)
{
	...
	if(bQuotes) QuoteString(strVoteAmount, sizeof(strVoteAmount));
	...
	if(bQuotes) QuoteString(strVoteRequired, sizeof(strVoteRequired));
	ReplaceString(strBuffer, iBufferSize, "{VOTE_REQUIRED}", strVoteRequired, false);
}

stock FormatTargetString(iVote, bool:bQuotes = false, iTarget, String:strBuffer[], iBufferSize)
{
	...
	if(!IsValidClient(iTarget))
	{
		...
		if(bQuotes) QuoteString(strAntiGrief, sizeof(strAntiGrief));
	...
	if(bQuotes) QuoteString(strTarget, sizeof(strTarget));
	...
	if(bQuotes) QuoteString(strTargetId, sizeof(strTargetId));
	ReplaceString(strBuffer, iBufferSize, "{TARGET_ID}", strTargetId, false);

	decl String:strTargetSteamId[MAX_NAME_LENGTH];
	GetClientAuthString(iTarget, strTargetSteamId, sizeof(strTargetSteamId));

	if(bQuotes) QuoteString(strTargetSteamId, sizeof(strTargetSteamId));
	...
	if(bQuotes) QuoteString(strTargetName, sizeof(strTargetName));
	ReplaceString(strBuffer, iBufferSize, "{TARGET_NAME}", strTargetName, false);
}

stock FormatMapString(iVote, bool:bQuotes = false, iMap, String:strBuffer[], iBufferSize)
{
	decl String:strMap[MAX_NAME_LENGTH];
	GetArrayString(g_hArrayVoteMapList[iVote], iMap, strMap, sizeof(strMap));

	if(bQuotes) QuoteString(strMap, sizeof(strMap));
	ReplaceString(strBuffer, iBufferSize, "{MAP_NAME}", strMap, false);

	decl String:strCurrentMap[MAX_NAME_LENGTH];
	GetCurrentMap(strCurrentMap, sizeof(strCurrentMap));

	if(bQuotes) QuoteString(strCurrentMap, sizeof(strCurrentMap));
	ReplaceString(strBuffer, iBufferSize, "{CURRENT_MAP_NAME}", strCurrentMap, false);
}

stock FormatOptionString(iVote, bool:bQuotes = false, iOption, String:strBuffer[], iBufferSize)
{
	decl String:strOptionName[MAX_NAME_LENGTH];
	GetArrayString(g_hArrayVoteOptionName[iVote], iOption, strOptionName, sizeof(strOptionName));

	if(bQuotes) QuoteString(strOptionName, sizeof(strOptionName));
	...
	if(bQuotes) QuoteString(strOptionResult, sizeof(strOptionResult));
