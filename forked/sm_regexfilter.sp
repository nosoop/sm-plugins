// Forked from https://forums.alliedmods.net/showthread.php?t=71867
// -- replaced function call to use Simple Chat Processor
...
public Action:OnChatMessage(&author, Handle:recipients, String:name[], String:message[]) {
		...
		if( (val > 0) && (ret == REGEX_ERROR_NONE) ) {
			...
			if (GetTrieValue(CurrInfo, "limit", val)) {
				...
				if(GetTrieValue(CurrInfo, "forgive", mod)) {
					...
					if(!GetTrieValue(ClientLimits[author], strval, any:datiem)) {
						datiem = GetGameTime();
						SetTrieValue(ClientLimits[author], strval, any:datiem)
					}	
	...
	if (changed) {
        strcopy(message, MAXLENGTH_MESSAGE-MAXLENGTH_NAME, text);
	}
...
stock LoadExpressions(String:file[], bool:optional = false) {
	...
	if(!FileExists(regexfile) && !optional) {
