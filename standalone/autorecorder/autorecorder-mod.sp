...
new g_iDemoStartTick = -1;

public Plugin:myinfo = {
    name = "Auto Recorder Mod",
    author = "Stevo.TVR",
...
public StartRecord()
{
    if(GetConVarBool(g_hTvEnabled) && !g_bIsRecording)
    {
        ...
        g_bIsRecording = true;
        g_iDemoStartTick = RoundToFloor(GetTickedTime() / GetTickInterval());
...
public GetApproximateDemoTick() {
    if (g_bIsRecording && g_iDemoStartTick > -1) {
        return RoundToFloor(GetTickedTime() / GetTickInterval()) - g_iDemoStartTick;
    } else {
        return 0;
    }
}
